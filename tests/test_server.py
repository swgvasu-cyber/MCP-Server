...
async def test_scan_hash_success():
    ...
    with patch('test.post_json', new_callable=AsyncMock) as mock_post:
        ...
        scan_hash_fn = mcp._tools['scan_hash'].fn
        result = await scan_hash_fn("e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855")
...
async def test_scan_hash_backend_error():
    with patch('test.post_json', new_callable=AsyncMock) as mock_post:
        ...
        scan_hash_fn = mcp._tools['scan_hash'].fn
        result = await scan_hash_fn("abc123")
